"""Arya Krishnagiri. This code contains two class,
Appt for single appointment conflicts and comparisons and Agenda for a list of appointments."""
...
class Appt:
    ...
    def intersect(self, other: 'Appt') -> 'Appt':
        ...
        return Appt(max(self.start, other.start), min(self.finish, other.finish), "Oh no, there is a conflict in the schedule")
        ...
        for i in range(len(self.elements)):
            for e in range(i + 1, len(self.elements)):
                if self.elements[i].overlaps(self.elements[e]):
                    conflicts.append(self.elements[i].intersect(self.elements[e]))
                else:
                    break
        return conflicts
